...
class TinybertConfig(object):
    """Configuration for `TinybertModel`.

    The default settings match the configuration of model `Tinybert_xxlarge`.
    """
    ...
    def from_dict(cls, json_object):
        ...
        config = TinybertConfig(vocab_size=None)
...
def get_tinybert_model(input_word_ids,
                   input_mask,
                   input_type_ids,
                   config=None,
                   name=None,
    ...
    bert_model_layer = TinybertModel(config=config, float_type=float_type, name=name)
...
class TinybertModel(tf.keras.layers.Layer):
    def __init__(self, config, float_type=tf.float32, **kwargs):
        super(TinybertModel, self).__init__(**kwargs)
        self.config = (TinybertConfig.from_dict(config)
    ...
    def build(self, unused_input_shapes):
        ...
        self.encoder = Transformer(
            ...
            shared_type = self.config.shared_type,
            name="encoder")
        ...
        super(TinybertModel, self).build(unused_input_shapes) 
    ...
    def __call__(self,
        ...
        return super(TinybertModel, self).__call__(inputs, **kwargs)
    
    def call(self, inputs, mode="tiny", **kwargs):
    ...
    def get_config(self):
        ...
        base_config = super(TinybertModel, self).get_config()
    ...
    def build(self, unused_input_shapes):
        ...
        if self.shared_type == "all":
            self.shared_layer = TransformerBlock(
                ...
                float_type=self.float_type,
                name=("shared_layer"))
            self.shared_layers = [self.shared_layer] * self.num_hidden_layers
        else:
            for i in range(self.num_hidden_layers):
                self.shared_layers.append(TransformerBlock(
                    hidden_size=self.hidden_size,
                    num_attention_heads=self.num_attention_heads,
                    intermediate_size=self.intermediate_size,
                    intermediate_activation=self.intermediate_activation,
