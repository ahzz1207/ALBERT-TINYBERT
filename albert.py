...
def get_albert_model(input_word_ids,
                   input_mask,
                   input_type_ids,
                   config=None,
                   name=None,
  ...
  bert_model_layer = AlbertModel(config=config, float_type=float_type, name=name)
...
class Transformer(tf.keras.layers.Layer):
  ...
  def call(self, inputs, return_all_layers=True, **kwargs):
    ...
    for i in range(self.num_hidden_layers):
      output_tensor, attention_score = self.shared_layers[i](output_tensor, attention_mask,**kwargs)
