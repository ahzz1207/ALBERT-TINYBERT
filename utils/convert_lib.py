...
def create_v2_checkpoint(model, src_checkpoint, output_path):
  ...
  #
  checkpoint = tf.train.Checkpoint(model=model)
  latest_checkpoint_file = tf.train.latest_checkpoint("/work/ALBERT-master_google")
  status = checkpoint.restore(latest_checkpoint_file).expect_partial()
  status.assert_existing_objects_matched().expect_partial()
  checkpoint.save("/work/ALBERT-TF2.0-master/model_configs/base/")
  model.save_weights("/work/ALBERT-TF2.0-master/model_configs/base/albert.h5")
  print("Done")
