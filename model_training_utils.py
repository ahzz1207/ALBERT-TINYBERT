...
def run_customized_training_loop(
    ...
    albert=None,
    tinybert=None,
    start_wtih_trained_model=False,
    loss_fn=None,
    # albert_model_dir=None,
    # tinybert_model_dir=None,
    model_dir=None,
    train_input_fn=None,
  ...
  Arguments:
      ...
      loss_fn: Function with signature func(labels, logits) and returns a loss
        tensor.
      albert_model_dir: albertModel directory used during training for restoring/saving model
        weights.
      tinybert_model_dir: tinybertModel directory used during training for restoring/saving model
        weights.
  ...
  with strategy.scope():
    ...
    # 从h5文件恢复模型
      ...
      if not os.path.exists(f"{model_dir}/models/"):
        raise ValueError("Model file is not exist!!!")
      albert.load_weights(f"{model_dir}/models/albert_model.h5")
      # 恢复tinybert
      if not os.path.exists(f"{model_dir}/models/tinybert_model.h5"):
        logging.info("Not find tinybert h5 file, restart from beginning")
      else:
